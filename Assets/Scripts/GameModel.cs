...
public class GameModel:IGameModel, IControllerGameModel
{
    ...
    {
        return _targets.SelectMany(x => x).Where(target => target.Id == targetId).FirstOrDefault();
    }

    public int GetTotalTargetsAmount()
    {
        return _targets.SelectMany(x => x).Count();
    }
    ...
    {
        return _targets.SelectMany(target => target).Where(target => !target.IsDestroyed.Value).ToList();
    }
